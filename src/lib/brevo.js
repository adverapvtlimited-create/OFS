...
/**
 * Dispatches a Career / Job Application email to HR and sends an acknowledgment email to the applicant via Brevo REST API
 * @param {Object} params
 * @param {Object} params.application - Candidate application data
 * @param {Object|null} params.file - Attached resume { buffer, base64, filePath, fileName, size, contentType, cloudinaryUrl }
 * @returns {Promise<{ success: boolean, messageId?: string, error?: string }>}
 */
export async function sendCareerApplicationEmail({ application, file = null }) {
  ...
  const emailSubject = `New Job Application: ${jobTitle} - ${candidateName} (${id})`;

  // Build attachments for Brevo
  const attachments = [];
  if (file) {
    const attachName = file.fileName || file.name || resumeName || 'Candidate_Resume.pdf';
    if (file.buffer && Buffer.isBuffer(file.buffer)) {
      attachments.push({
        content: file.buffer.toString('base64'),
        name: attachName,
      ...
      try {
        ...
        } else if (file.cloudinaryUrl || cloudinaryUrl) {
          attachments.push({
            url: file.cloudinaryUrl || cloudinaryUrl,
            name: attachName,
          });
        }
      } catch (err) {
        console.warn('[Brevo Resume Attachment Warning]:', err.message);
        if (file.cloudinaryUrl || cloudinaryUrl) {
          attachments.push({
            url: file.cloudinaryUrl || cloudinaryUrl,
            name: attachName,
          });
  ...
  const effectiveResumeUrl = file?.cloudinaryUrl || cloudinaryUrl || application.resumeUrl;

  // Admin / HR Notification HTML Template
  const hrHtmlContent = `
<!DOCTYPE html>
<html>
<head>
...
<body style="margin: 0; padding: 0; background-color: #f1f5f9; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #1e293b;">
        ...
        <table role="presentation" width="100%" style="max-width: 640px; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.06); border: 1px solid #e2e8f0;">
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                  <td>
                    ...
                    <h1 style="margin: 0; color: #ffffff; font-size: 22px; font-weight: 700; line-height: 1.3;">
                      New Candidate Application: ${jobTitle}
                    </h1>
                    <p style="margin: 6px 0 0 0; color: #94a3b8; font-size: 13px;">
                      Ref ID: <strong style="color: #f8fafc; font-family: monospace;">${id}</strong> &bull; Received on ${timestamp} IST
                    </p>
          ...
          <tr>
            <td style="padding: 30px;">
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-bottom: 24px; font-size: 14px;">
                <tr>
                  <td style="padding: 8px 0; width: 38%; color: #64748b; font-weight: 600;">Full Name:</td>
                  <td style="padding: 8px 0; color: #0f172a; font-weight: 700;">${candidateName}</td>
                </tr>
                <tr style="border-top: 1px dashed #f1f5f9;">
                  <td style="padding: 8px 0; color: #64748b; font-weight: 600;">Email Address:</td>
                  <td style="padding: 8px 0;">
                    <a href="mailto:${candidateEmail}" style="color: #dc2626; font-weight: 600; text-decoration: none;">${candidateEmail}</a>
                  </td>
                </tr>
                <tr style="border-top: 1px dashed #f1f5f9;">
                  <td style="padding: 8px 0; color: #64748b; font-weight: 600;">Phone / Mobile:</td>
                  <td style="padding: 8px 0;">
                    <a href="tel:${phone}" style="color: #0f172a; font-weight: 600; text-decoration: none;">${phone}</a>
                  </td>
                </tr>
                <tr style="border-top: 1px dashed #f1f5f9;">
                  ...
                  <td style="padding: 8px 0; color: #071330; font-weight: 700;">${jobTitle}</td>
                </tr>
                <tr style="border-top: 1px dashed #f1f5f9;">
                  <td style="padding: 8px 0; color: #64748b; font-weight: 600;">Total Experience:</td>
                  <td style="padding: 8px 0; color: #0f172a; font-weight: 600;">${experienceYears}</td>
                </tr>
                <tr style="border-top: 1px dashed #f1f5f9;">
                  <td style="padding: 8px 0; color: #64748b; font-weight: 600;">Current / Last Company:</td>
                  <td style="padding: 8px 0; color: #0f172a; font-weight: 600;">${currentCompany}</td>
                </tr>
              </table>

              <!-- Cover Note -->
              <h3 style="margin: 0 0 10px 0; color: #071330; font-size: 15px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 700; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px;">
                📝 Candidate Cover Note &amp; Highlights
              </h3>
              <div style="background-color: #f8fafc; border-left: 4px solid #dc2626; padding: 16px; border-radius: 0 8px 8px 0; font-size: 14px; line-height: 1.6; color: #334155; margin-bottom: 24px; white-space: pre-wrap;">
                ${coverNote ? coverNote.replace(/</g, '&lt;').replace(/>/g, '&gt;') : '<em>No cover note provided by candidate.</em>'}
              </div>
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top: 10px;">
                <tr>
                  <td align="center">
                    <a href="mailto:${candidateEmail}?subject=RE:%20Application%20for%20${encodeURIComponent(jobTitle)}%20at%20${encodeURIComponent(senderName)}%20(${encodeURIComponent(id)})" style="display: inline-block; background-color: #071330; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 6px; font-size: 14px; font-weight: 700; letter-spacing: 0.5px;">
                      Reply directly to ${candidateName} (${candidateEmail})
                    </a>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="background-color: #f8fafc; padding: 20px 30px; border-top: 1px solid #e2e8f0; text-align: center; font-size: 12px; color: #94a3b8;">
              <p style="margin: 0 0 4px 0; font-weight: 600; color: #64748b;">
                ${senderName} &bull; Talent Acquisition Desk
              </p>
              <p style="margin: 0;">
                Careers Mailbox: <a href="mailto:${receiverEmail}" style="color: #dc2626; text-decoration: none;">${receiverEmail}</a>
              </p>
            </td>
          </tr>
  ...
  try {
    ...
    if (!response.ok) {
      console.error('[Brevo Career API Error]:', result);
      return {
        success: false,
        error: result.message || 'Failed to dispatch career application email via Brevo',
      };
    }
    ...
    console.error('[Brevo Career Network Error]:', err);
    return {
      success: false,
      error: err.message || 'Internal connection error communicating with Brevo',
    };
...
/**
 * Sends automated receipt acknowledgment email to the candidate via Brevo
 */
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #1e293b; background-color: #f8fafc; padding: 25px;">
  <div style="max-width: 580px; margin: 0 auto; background: #ffffff; border-radius: 8px; border: 1px solid #e2e8f0; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
    <div style="background-color: #071330; padding: 24px; color: #ffffff; border-bottom: 3px solid #dc2626;">
      ...
      <p style="margin: 4px 0 0 0; color: #94a3b8; font-size: 13px;">Application Reference ID: <strong>${applicationId}</strong></p>
    </div>
    <div style="padding: 24px; line-height: 1.6; font-size: 14px;">
      <p>Dear <strong>${candidateName}</strong>,</p>
      <p>Thank you for your interest in joining <strong>${senderName}</strong> and applying for the position of <strong>${jobTitle}</strong>.</p>
      <p>We have successfully received your candidate profile and resume. Our Talent Acquisition team is currently reviewing your qualifications and professional background.</p>
      <div style="background: #f1f5f9; padding: 14px 18px; border-radius: 6px; margin: 18px 0;">
        <p style="margin: 0; font-size: 13px; color: #475569;">
          ⏱ <strong>Hiring Review SLA:</strong> If your qualifications match our current requirements, a member of our talent team will reach out to schedule an interview within <strong>3-5 business days</strong>.
        </p>
      </div>
      <p>If you have any questions or need to provide updated credentials, please feel free to reach out to our talent desk at <a href="mailto:${receiverEmail}" style="color: #dc2626; font-weight: 600;">${receiverEmail}</a>.</p>
      <p style="margin-top: 24px; color: #64748b; font-size: 13px;">Warm regards,<br><strong style="color: #071330;">Talent Acquisition &amp; People Operations</strong><br>${senderName}</p>
    </div>
  </div>
</body>
</html>
